...
public class PersonaController extends GenericController<Persona, PersonaDAO> {
    ...
    public ResponseEntity<?> buscarPorNombreYApellido(@RequestParam String nombre, @RequestParam String apellido){
        ...
        if (!oPersona.isPresent()){
            //throw new BadRequestExeption(String.format("No se encotro Persona con nombre %s y apellido %s",nombre,apellido));
            mensaje.put("success",Boolean.FALSE);
            mensaje.put("success",String.format("No se encotro Persona con nombre %s y apellido %s",nombre,apellido));
